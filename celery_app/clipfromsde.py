...
def clipfromsde(mapnumlist,MEDIA,taskname,taskpackage_id,taskpackageson_id):
    ...
    changeDJdbtasktable(dbname,tablename,taskpackage_id,statusfieldname,filefieldname,MEDIAfilepath,taskpackageson_id,taskpackagesontablename)
    changSDEmapindex(mapnumlist, mapindexsde)
    return True

def changeDJdbtasktable(dbname,tablename,taskpackage_id,statusfieldname,filefieldname,MEDIAfilepath,taskpackageson_id,taskpackagesontablename):
    ...
    #print "tt"
    cur = conn.cursor()
    UPDATESQL=u"UPDATE %s set %s = '1',%s='%s' where ID=%d" %(tablename,statusfieldname,filefieldname,MEDIAfilepath,taskpackage_id)
    #SQL="SELECT * FROM %s" %tablename
    #print UPDATESQL
    cur.execute(UPDATESQL)

    UPDATESQL1=u"UPDATE %s set %s='%s' where ID=%d" %(taskpackagesontablename,filefieldname,MEDIAfilepath,taskpackageson_id)
    #SQL="SELECT * FROM %s" %tablename
    #print UPDATESQL
