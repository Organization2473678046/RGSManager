...
class TaskPackageSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        ...
        gbrgssdepath = regiontask.gbrgssde
        jbrgssdepath = regiontask.jbrgssde

        if mapindexsdepath is not None and gbrgssdepath is not None and jbrgssdepath is not None:
            if os.path.exists(mapindexsdepath) and os.path.exists(gbrgssdepath) and os.path.exists(jbrgssdepath):
                # 进入celery进行作业包的异步裁切
                clipfromsde.delay(mapindexsdepath, gbrgssdepath, jbrgssdepath, mapnumlist, MEDIA, taskname, taskpackage.id,
                                  taskpackageson.id)
