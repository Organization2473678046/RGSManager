...
class Test_taskpackageSon(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.base_url = u"http://192.168.3.120:8000/v8/taskpackagesons/?regiontask_name=东南区域1800幅&taskpackage_name=task1"
        self.base_url2 = u"http://192.168.3.120:8000/v8/taskpackagesons/?regiontask_name=东南区域1800幅&taskpackage_name=task2"
        ...
        self.data_dict_admin = data_dict_admin
        self.data_dict_worker = data_dict_worker
        self.file_path = "C:\\Users\\Administrator\\Desktop\\chardet-master.zip"
        self.reponse_list_admin = ["201", self.data_dict_admin["taskpackage_name"], self.data_dict_admin["regiontask_name"],
                             self.data_dict_admin["describe"], self.data_dict_admin["user_username"], self.data_dict_admin["reallyname"], self.data_dict_admin["schedule"]]
        self.reponse_list_worker = ["201", self.data_dict_worker["taskpackage_name"], self.data_dict_worker["regiontask_name"],
                             self.data_dict_worker["describe"], self.data_dict_worker["user_username"], self.data_dict_worker["reallyname"], self.data_dict_worker["schedule"]]


    def test_taskpackageson_admin(self):
        """验证管理员创建上传记录，对比记录准确性,管理员root,任务包task1,验证登出后无权访问数据"""
        driver = self.driver
    ...
    def test_taskpackageson_worker_false(self):
        """验证作业员上传不属于自己的任务包,作业员worker,任务包task1，验证登出后权限"""
        driver = self.driver
        driver.get(self.base_url)
        # 作业员登录
        driver.find_element_by_link_text("Log in").click()
    ...
    def test_taskpackageson_worker_true(self):
        """验证作业员上传属于自己的作业包,作业员worker,任务包task2"""
        driver = self.driver
        driver.get(self.base_url)
        # 作业员登录
        driver.find_element_by_link_text("Log in").click()
